...
public class RegexLogParser implements LogParser {
    ...
    private static final Pattern STACK_TRACE_PATTERN = Pattern.compile("详细信息:\\s*\\[(.*?)]", Pattern.DOTALL);
    ...
    private List<StackFrame> parseStackTrace(String rawLog) {
        ...
        Matcher stackTraceMatcher = STACK_TRACE_PATTERN.matcher(rawLog);
        if (stackTraceMatcher.find()) {
            String stackTraceContent = stackTraceMatcher.group(1);
            String[] frames = FRAME_SPLITTER.split(stackTraceContent);
            ...
            for (String frameString : frames) {
                ...
                if (frameMatcher.find()) {
                    ...
                    frame.businessFlag = fullMethodPath.startsWith("com.dyyl"); // 可配置
